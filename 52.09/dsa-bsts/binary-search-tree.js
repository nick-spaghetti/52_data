...
class BinarySearchTree {
	...
	isBalanced(current = this.root) {
		...
		const minDepth = (current) => {
			...
			return (
				1 + Math.min(minDepth(current.left), minDepth(current.right))
			);
		};
		const maxDepth = (current) => {
			if (current === null) return 0;
			return (
				1 + Math.max(maxDepth(current.left), maxDepth(current.right))
			);
		};
	}
	...
	findSecondHighest(current = this.root) {
		...
		while (current) {
			...
			if (current.right && !current.left) return current.val;
			current = current.right;
		}
	}
